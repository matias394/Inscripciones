...
export class ButtonFileUploaderComponent implements OnInit, ControlValueAccessor {
  ...
  uploadFile(event: any) {
    ...
    reader.onloadend = (event) => {
      var data = event.target.result; // data binaria a enviar a S3
  ...
  private archivoValido(file): boolean {
    ...
    if (this.maxSizeMB) {
      const bytesTamaoMaximoArchivo = this.maxSizeMB * 1024 * 1024;
      if (file.size > bytesTamaoMaximoArchivo) {
        // emitimos un error de tama�o maximo superado
