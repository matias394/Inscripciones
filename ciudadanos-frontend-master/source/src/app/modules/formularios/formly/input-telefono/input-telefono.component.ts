...
export class InputTelefonoComponent extends FieldType implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.etiqueta = `${this.field.templateOptions.label}${(this.field.templateOptions.required) ? '*' : ''}`;
    ...
    this.previsualizacion = JSON.stringify(perfilMiba) === JSON.stringify(PERFIL_MIBA_PREVISUALIZACION);

    let valueTelefono = this.formControl.value;
    if (this.editarEnMiba || (this.field.templateOptions['noEditableEnGrupo'] && this.field.templateOptions['enEdicion'])) {
    ...
    if (this.editarEnMiba) {
      ...
      if (this.field.templateOptions.required) {
        ...
        this.camposRequeridosMibaService.addCampoRequeridoMiba(`area_code_${tipo}`);
    ...
    this.formTelefono.valueChanges.pipe(
      startWith(null)
    ).subscribe(
  ...
  validarTelefono(val: {
    ...
    const datosValidos = Boolean(val.codigopais) && Boolean(val.codigoarea) && Boolean(val.numerotelefono);
    return (this.formTelefono.disabled && datosValidos) ||
