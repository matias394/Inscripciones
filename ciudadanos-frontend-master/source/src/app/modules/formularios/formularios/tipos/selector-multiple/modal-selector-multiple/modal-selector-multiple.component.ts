...
@Component({
  selector: 'app-modal-selector-multiple',
  templateUrl: './modal-selector-multiple.component.html',
  styleUrls: ['./modal-selector-multiple.component.css']
})
export class ModalSelectorMultipleComponent extends ModalComponenteFormulario implements OnInit {
  ...
  override initModal(tab: 'general' | 'validaciones' | 'visibilidad' | 'conceptos', modelo?: ModeloCampoFormulario) {
    super.initModal(tab, modelo);
    this.opcionesListaValores.seleccionMultiple = true;
    this.opcionesListaValores.incializarValoresPorDefecto(this.modelo['valoresPorDefecto']);
    this.conceptosPagoComponent.actualizarOpcionesDisponibles(this.modelo['valores']);
  }

  getInstancia(): ModeloCampoFormulario {
