...
export class TextboxSimpleComponent
  extends FieldType
  implements OnInit, OnDestroy
{
  ...
  ngOnInit() {
    ...
    this.editarEnMiba =
      this.field.templateOptions['mibaKey'] &&
      this.field.templateOptions['editable'];

    const perfilMiba = this.localAndSesionStorageService.getElement(
      this.localAndSesionStorageService.PROFILE_MIBA
    );
    this.previsualizacion =
      perfilMiba === JSON.stringify(PERFIL_MIBA_PREVISUALIZACION);
  ...
  assingSelf(): void {
    ...
    if (
      Boolean(mibaKey) ||
      (!this.field['editableOperador'] && (this.nuevaInscripcion == null || !this.nuevaInscripcion))
    ) {
      this.readonly = true;
    }

    if (mibaKey) {
      const mibaProfile = JSON.parse(
        this.localAndSesionStorageService.getElement(
          this.localAndSesionStorageService.PROFILE_MIBA
        )
      );
