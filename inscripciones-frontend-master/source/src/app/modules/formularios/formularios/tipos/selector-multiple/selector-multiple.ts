...
export class SelectorMultiple extends ModeloCampoFormulario {
  ...
  setConfiguracionFormly(config: SelectorMultipleConfigFormly): void {
    ...
    const opciones = (config.templateOptions.opciones || []).map(v => v.descripcion);
    this.datosGenerales = {
      nombre: config.name,
      etiqueta: config.templateOptions.label,
      descripcion: config.templateOptions.description,
      valoresPorDefecto: (config.templateOptions.opciones || []).filter(v => v.valorPorDefecto).map(v => v.descripcion),
      valores: opciones,
